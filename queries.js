...
const runWorkflowByOrder = async (request, response) => {
    ...
    console.log("nodesOrder", nodesOrder.length, nodes.length)
    let username = ""
...
const getNodesOrder = (n, e) => {
    ...
    for (let i = 0; i < edges.length; i++) {
        for (let j = 0; j < nodes.length; j++) {
            if (edges[i].target === nodes[j].id && nodes[j].isAdded === false) {
                const payload = {
                    id: nodes[j].id,
                    type: nodes[j].type,
                    data: nodes[j].data,
        ...
        if (i === edges.length - 1) {
            for (let k = 0; k < nodes.length; k++) {
                if (edges[i].source === nodes[k].id && nodes[k].isAdded === false) {
                    const payload = {
                        id: nodes[k].id,
                        type: nodes[k].type,
                        data: nodes[k].data,
